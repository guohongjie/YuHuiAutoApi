		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="static/img/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
	            ...
	            $(".upload_box").append(html).css({"width":para.width,"height":para.height});
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "static/img/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						runButton="<button class=\"btntest\" id=\"btn_test\" style=\"margin-right: 30px\" onclick=\"runSuiteApi()\">运行测试</button>"
						$("body").append("<p>上传成功，文件地址是：" + response + "</p>"+runButton);
						//$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>"+runButton);
