		...
		path : {
			...
			props: {
				...
				xpath: {
					...
					editor : function() {
						//return new a.editors.textEditor()
						return new a.editors.inputEditor();
					}
				}
			}
		},
...
//
			...
			var wH=svg.getAttribute('height')-100;
			var wW=svg.getAttribute('width')-200;
			if(r>wH){   //y轴
				svg.setAttribute('height', r+150)
//				svg.setAttribute('height', 1371)
			}
			if(F>wW){   //x轴
				svg.setAttribute('width', F+150)
//				svg.setAttribute('width', F+150)
			}
...
//				r += "\""+o + "\":{\"value\":\"" + escape(E.props[o].value) + "\"},"
//				r += "\""+o + "\":{\"value\":\'" + E.props[o].value + "\'},"
//                if (E.props[o].name=="params"){
                    r += "\""+o + "\":{\"value\":\"" + E.props[o].value.replace(/\"/g,"'") + "\"},"
//                }else{
//				    r += "\""+o + "\":{\"value\":\'" + E.props[o].value + "\'},"
//                    }
			}
...
//					alert(d[e].value);
	...
	a.init = function(x, r) {
		...
		b(document).keydown(function(i,z) {
		...
		b(y).bind(
				"addrect",
				function(j, h, k, wc, wcname, autoParams) {  // wc 是api的id  wcname 接口名称
					//限定 开始  结束 只能有一个节点
					var i = "{\"states\":{";
					for ( var c in q) {
						if (q[c]) {
					...
					for( keys in _data.states){
						...
						val_ary.push(items.type);
					}

					if(val_ary.indexOf('start')!=-1&&h=='start'){
                        return
					}
					if(val_ary.indexOf('end')!=-1&&h=='end'){
						return
					}
					if (h!='start'&&h!='end'&&h!='error'){
					a.config.tools.states[h].text={"text":wcname};    //在这里新增则显示更改
					a.config.tools.states[h].props.api_id.value=wc;    //在这里新增则显示更改
					a.config.tools.states[h].props.params.value=autoParams.context.childNodes[5].innerText;    //在这里新增则显示更改
					}

					var z = new a.rect(b.extend(true, {},
							a.config.tools.states[h], k), y);
					q[z.getId()] = z;
					console.log(a.config.tools.states[h])
		...
		if (a.config.editable) {
			...
			}).css({left:"1px",top:"60px"});
            b("#helpDiv").draggable({
                    handle : "#helpDiv_title"
                }).css({left:"1111px",top:"60px"});
            b("#testDiv").draggable({
                    handle : "#testTitle"
                }).css({left:"815px",top:"60px"});
            b("#get_params").draggable({
                    handle : "#get_params_title"
                }).css({left:"1313px",top:"60px"});
			...
			b(x).droppable({   //这里拖动触发画布
				accept : ".state",
				drop : function(c, i) {
//				console.log(i.helper.attr("value"));    //这里获取 接口id
//				console.log(i.helper.context.innerText);    //这里获取 接口名称
					b(y).trigger("addrect", [
					i.helper.attr("type"),
					 {attr : {
						...
						i.helper.attr("value"),  //接口id
						i.helper.context.innerText,  //接口名称
						i.helper
					])
				}
			});
			b("#ptext input,#pshow input").live('change',function(e){   //名称的显示校验
				...
				//任务 子流程 自动 的名称不允许重复
				if(_val!="开始"||_val!="结束"||_val!="错误"||_val!="任务"){
					var count=0;
					for( keys in _data.states){
						var items=_data.states[keys];
						val_ary.push(items.text.text);
						if(items.text.text==_val){
							count++
						}
					}
				}
				...
				if(count>=2){
					alert("警告:"+_val+'命名重复，请修改名称')
				}

			});

			b("#pforeachlist input").live('change',function(e){   //动态分支的集合校验
...
//							alert(q[c].toJson());
						...
						suiteDatas = i.replace(/[\r\n]/g,"").replace(/\ +/g,"");
                         var _data = eval("(" + suiteDatas + ")");
                        ...
                        var boolSaveUpdate = $(".newSuitName").css("display");
                        if (boolSaveUpdate=="none"){
                            var saveUpdate = 0;
                            var flowdata = $("#file").find("option:selected")
                            var suiteId = $("#file").find("option:selected").val();
                            var flowName = $("#file").find("option:selected").attr("text");
                        }
                        else{
                            var saveUpdate = 1;    // 新增
                            var flowName = $(".suitName").val();
                            var suiteId = "99999999999999";
                        }
                        if ($.isEmptyObject(_data['states'])&&$.isEmptyObject(_data['paths'])){
                            return;
                        }
                        if (flowName=="选择流程图"){
                            return;
						...
						$.ajax({
                            ...
                            type: "post",
                            data: {
                                datas: JSON.stringify(suiteDatas),
                                saveUpdate: saveUpdate,
                                flowName: flowName,
                                suiteId: suiteId,
                                },
                            success: function (returndata) {
                                    alert(returndata["datas"]);
...
//							alert(q[c].toJson());
                        ...
                        new_suiteDatas = suiteDatas.replace(/\s*/g,"")
						$.ajax({
                            url: "/testSuite",
                            type: "post",
                            data: {
                                datas: JSON.stringify(new_suiteDatas),
                                isSuiteOrigin: "1",
                                },
                            success: function (returndata) {
//                                    alert(returndata["datas"]);
                                        ...
                                        for (var i=0; i<returndata["datas"].length; i++){
                                            if (returndata["datas"][i][3]!="Success"){
                                                    var color="red";
                                             }else{
                                                    var color="black";
                                                 }
                                            $("#testResult").append("<h4 style='padding:40px 6px 0px 50px;color:"+color+";'>请求接口:"+returndata["datas"][i][0]+"</h4>");
                                            $("#testResult").append("<p style='padding:10px 6px 0px 50px;color:"+color+";'>请求信息:"+returndata["datas"][i][1]+"</p>");
                                            $("#testResult").append("<p style='padding:10px 6px 0px 50px;color:"+color+";'>测试状态:"+returndata["datas"][i][3]+"</p>");
                                            $("#testResult").append("<p style='padding:10px 6px 0px 50px;color:"+color+";'>测试信息:"+returndata["datas"][i][2]+"</p>");
                                                }
                                        },
                            error: function (returndata) {
                                    alert("操作失败！")
		...
		var A = a.config.historyRects, l = a.config.activeRects,p=a.config.markRects;
		if (A.rects.length || l.rects.length|| p.rects.length) {
			...
			for (var u = 0; u < p.rects.length; u++) {
				if (z[p.rects[u].name]) {
					z[p.rects[u].name].rect.attr(p.rectAttr)
				}
				for (var t = 0; t < p.rects[u].paths.length; t++) {
					if (z[p.rects[u].name].paths[p.rects[u].paths[t]]) {
						z[p.rects[u].name].paths[p.rects[u].paths[t]]
							.attr(p.pathAttr)
					}
				}
			}
...
//                        if (boolSaveUpdate=="none"){
//                            var flowdata = $("#file").find("option:selected")
//                            var suiteId = flowdata.val();
//                            var flowName = flowdata.attr("text");
//                        }
//                        else{
//                            var saveUpdate = 1;
//                            var flowName = $(".suitName").val();
...
//			);
	...
	b.fn.myflow = function(c,ls="None") {
