...
$.extend(true, myflow.editors, {
	inputEditor : function(){
		...
		this.init = function(props, k, div, src, r){  //点击触发
			_props=props; _k=k; _div=div; _src=src; _r=r;
			if (props[_k].name!="params"){
			$('<input style="width:100%;"/>').val(props[_k].value).change(function(){
				props[_k].value = $(this).val();
			}).appendTo('#'+_div);
			}else{
			    $('<textarea style="width:100%;height:300px"></textarea>').val(unescape(props[_k].value)).change(function(){
				props[_k].value = $(this).val();
			}).appendTo('#'+_div);
			}
